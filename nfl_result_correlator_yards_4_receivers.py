...
for team in team_data:
    ...
    for event in team_data[team]:
        #print('event',event)
        if 'Pass Yards' in event and 'Receiving Yards' in event and len(event['Pass Yards']) == 1 and len(event['Receiving Yards']) > 3:      
         ...
         #   print('pass_result',pass_result)
            ...
            result = next(rriter)
            receiving_result3 = result['result']
            receiving_name3 = result['name']       
            result = next(rriter)
            receiving_result4 = result['result']
            receiving_name4 = result['name']             
            
            raw_total_events = raw_total_events + 1
            team_raw_total = team_raw_total + 1
            
            if pass_result == receiving_result1 and pass_result == receiving_result2 and pass_result == receiving_result3 and pass_result == receiving_result4:
                ...
                if pass_result == 'Under':
                    ...
                    team_matches.append((pass_name, receiving_name1, receiving_name2, receiving_name3, receiving_name4,'Under'))
                elif pass_result == 'Over':
                    raw_over_match_count = raw_over_match_count + 1 
                    team_raw_over = team_raw_over + 1
                    team_matches.append((pass_name, receiving_name1, receiving_name2, receiving_name3, receiving_name4, 'Over'))
